...
const merchantCreateSchema = z.object({
  ...
  payment_mode: z.enum(['stripe_direct', 'stripe_connect', 'manual', 'bank_transfer', 'cash']),
...
resellerRoutes.get('/stripe/status', async (c) => {
  const { reseller } = getResellerContext(c);
  const db = createServiceClient();
  const sum = (rows?: any[] | null) => (rows ?? []).reduce((total, row) => total + Number(row.reseller_commission_cents ?? 0), 0);
  let pendingCommissionCents = 0;
  let paidCommissionCents = 0;
  try {
    const [pendingRes, paidRes] = await Promise.all([
      db.from('reseller_commissions').select('reseller_commission_cents, currency').eq('reseller_id', reseller.id).eq('status', 'pending'),
      db.from('reseller_commissions').select('reseller_commission_cents, currency').eq('reseller_id', reseller.id).eq('status', 'paid'),
    ]);
    if (pendingRes.error) console.warn('[reseller] pending commission lookup failed:', pendingRes.error.message);
    else pendingCommissionCents = sum(pendingRes.data);
    if (paidRes.error) console.warn('[reseller] paid commission lookup failed:', paidRes.error.message);
    else paidCommissionCents = sum(paidRes.data);
  } catch (error) {
    console.warn('[reseller] commission lookup failed:', error instanceof Error ? error.message : error);
  }
  return c.json({
    data: {
      enabled: false,
      mode: 'stripe_direct_commissions',
      connected: false,
      reseller,
      pending_commission_cents: pendingCommissionCents,
      paid_commission_cents: paidCommissionCents,
      currency: reseller.currency,
      message: 'V1 active: Fidelopass encaisse les abonnements Stripe et suit votre marge pour un règlement manuel.',
    },
  });
});
...
resellerRoutes.post('/stripe/connect', async (c) => {
  return c.json({
    error: 'Stripe Connect n’est pas utilisé dans la V1. Fidelopass encaisse les abonnements et règle les commissions revendeur manuellement.',
  }, 410);
});
...
resellerRoutes.post('/merchants', async (c) => {
  ...
  const isOnlinePayment = parsed.data.payment_mode === 'stripe_direct' || parsed.data.payment_mode === 'stripe_connect';
  const shouldActivateNow = parsed.data.activate_now && !isOnlinePayment;
...
resellerRoutes.post('/merchants/:id/checkout', async (c) => {
  ...
  const merchant = await loadOwnedResellerMerchant(db, reseller.id, c.req.param('id'));
  if (!merchant) return c.json({ error: 'Commerçant introuvable.' }, 404);
  const onlinePayment = merchant.payment_mode === 'stripe_direct' || merchant.payment_mode === 'stripe_connect';
  if (!onlinePayment) return c.json({ error: 'Ce commerçant n’est pas en paiement Stripe Checkout.' }, 400);
...
resellerRoutes.post('/merchants/:id/manual-activate', async (c) => {
  ...
  const merchant = await loadOwnedResellerMerchant(db, reseller.id, c.req.param('id'));
  if (!merchant) return c.json({ error: 'Commerçant introuvable.' }, 404);
  if (merchant.payment_mode === 'stripe_connect' || merchant.payment_mode === 'stripe_direct') {
    return c.json({ error: 'Utilisez Stripe Checkout pour ce commerçant.' }, 400);
  }
...
resellerRoutes.post('/merchants/:id/change-plan', async (c) => {
  ...
  try {
    ...
    const { data, error } = await db.from('reseller_merchants').update({
      ...
      reseller_margin_cents: pricing.reseller_margin_cents,
      currency: pricing.currency,
      payment_mode: merchant.payment_mode === 'stripe_connect' ? 'stripe_direct' : merchant.payment_mode,
      updated_at: now,
